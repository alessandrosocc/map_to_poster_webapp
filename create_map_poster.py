...
FONTS_DIR = "fonts"
POSTERS_DIR = "posters"
CAPITALS_ARCHIVE_PATH = Path("data/capitals.json")
TOP_CITIES_ARCHIVE_PATH = Path("data/top_cities.json")

FILE_ENCODING = "utf-8"

FONTS = load_fonts()
CAPITALS_ARCHIVE = None
TOP_CITIES_ARCHIVE = None
...
def normalize_place_name(value: str) -> str:
    """
    Normalize city/country names for local archive matching.
    """
...
def load_capitals_archive():
    """
    Load the local capitals archive once.
    """
    global CAPITALS_ARCHIVE
    if CAPITALS_ARCHIVE is not None:
        return CAPITALS_ARCHIVE

    if not CAPITALS_ARCHIVE_PATH.exists():
        CAPITALS_ARCHIVE = []
        return CAPITALS_ARCHIVE

    try:
        with CAPITALS_ARCHIVE_PATH.open("r", encoding=FILE_ENCODING) as file:
            payload = json.load(file)
        CAPITALS_ARCHIVE = payload.get("capitals", [])
    except (OSError, json.JSONDecodeError) as exc:
        print(f"⚠ Could not load capitals archive: {exc}")
        CAPITALS_ARCHIVE = []

    return CAPITALS_ARCHIVE


def load_top_cities_archive():
    """
    Load the local top-cities archive once.
    """
    global TOP_CITIES_ARCHIVE
    if TOP_CITIES_ARCHIVE is not None:
        return TOP_CITIES_ARCHIVE

    if not TOP_CITIES_ARCHIVE_PATH.exists():
        TOP_CITIES_ARCHIVE = []
        return TOP_CITIES_ARCHIVE

    try:
        with TOP_CITIES_ARCHIVE_PATH.open("r", encoding=FILE_ENCODING) as file:
            payload = json.load(file)
        TOP_CITIES_ARCHIVE = payload.get("cities", [])
    except (OSError, json.JSONDecodeError) as exc:
        print(f"⚠ Could not load top cities archive: {exc}")
        TOP_CITIES_ARCHIVE = []

    return TOP_CITIES_ARCHIVE


def get_top_city_coordinates(city: str, country: str):
    """
    Return coordinates from the local top-cities archive when city/country match.
    """
    wanted_city = normalize_place_name(city)
    wanted_country = normalize_place_name(country)
    if not wanted_city or not wanted_country:
        return None

    for record in load_top_cities_archive():
        city_aliases = record.get("city_aliases") or [record.get("city", "")]
        country_aliases = record.get("country_aliases") or [record.get("country", "")]
        city_matches = wanted_city in {normalize_place_name(item) for item in city_aliases}
        country_matches = wanted_country in {normalize_place_name(item) for item in country_aliases}
        if city_matches and country_matches:
            try:
                ...
                return None
            return lat, lon, record.get("city", city), record.get("country", country)

    return None


def get_capital_coordinates(city: str, country: str):
    """
    Return coordinates from the local capitals archive when city/country match.
    """
    wanted_city = normalize_place_name(city)
    wanted_country = normalize_place_name(country)
    if not wanted_city or not wanted_country:
    ...
    for record in load_capitals_archive():
        capital_aliases = record.get("capital_aliases") or [record.get("capital", "")]
        country_aliases = record.get("country_aliases") or [record.get("country", "")]
        city_matches = wanted_city in {normalize_place_name(item) for item in capital_aliases}
        country_matches = wanted_country in {normalize_place_name(item) for item in country_aliases}
        if city_matches and country_matches:
            try:
                lat = float(record["latitude"])
                ...
                return None
            return lat, lon, record.get("capital", city), record.get("country", country)
...
def get_coordinates(city, country):
    ...
    top_city_match = get_top_city_coordinates(city, country)
    if top_city_match:
        lat, lon, city_name, country_name = top_city_match
        print(f"✓ Using local top-cities archive for {city_name}, {country_name}")
        try:
    ...
    capital_match = get_capital_coordinates(city, country)
    if capital_match:
        lat, lon, capital_name, country_name = capital_match
        print(f"✓ Using local capitals archive for {capital_name}, {country_name}")
        try:
            cache_set(coords, (lat, lon))
        except CacheError as e:
            print(e)
